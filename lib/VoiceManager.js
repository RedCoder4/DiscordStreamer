  ...
  start (list) {
    ...
    this.bot.joinVoiceChannel(this.options.vc).then((vc) => {
      ...
      if (this.list.length > 0) {
        if (!fs.existsSync(this.data.dir + '/discordstreamer/' + this.list[this.index].urlID + '.mp3')) {
          ytdl('https://www.youtube.com/watch?v=' + this.list[this.index].urlID).pipe(fs.createWriteStream(this.data.dir + '/discordstreamer/' + this.list[this.index].urlID + '.mp3'));
          this.vc.play(ytdl('https://www.youtube.com/watch?v=' + this.list[this.index].urlID));
        } else {
          this.vc.play(this.data.dir + '/discordstreamer/' + this.list[this.index].urlID + '.mp3');
        }
        formatArray = [];
