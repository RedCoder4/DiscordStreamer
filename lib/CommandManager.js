              ...
              if (r._responses.length === 0) {
                try {
                  ytdl.getInfo(suffix, (notUsed, vid) => {
                    ...
                    var title = vid.title;
                    var final = {"urlID": vid.vid, "name": title, "length": min + ':' + sec, "requester": msg.author.id};
                    // Requester is logged for moderation purposes.
                    ytdl(vid.vid, {audioonly: true}).on('error', (e) => {bot.createMessage(msg.channel.id, '**ERROR!** Invalid link or unable to fetch!');console.log(e);}).pipe(fs.createWriteStream(all.rawData.dir + all.rawData.options.saveDir + vid.vid + '.mp3'));
    ...
    this.bot.on('messageCreate', msg => {
      if (msg.content.startsWith(this.rawData.options.prefix) && msg.channel.id === this.rawData.options.feed) {
        ...
        if (commands.hasOwnProperty(base[0])) {
          ...
          if (suffix) output += ' ' + suffix;
          this.rawData.debugMsg(msg.author.username + '#' + msg.author.discriminator + ' executed <' + output + '>');
          aliases[base[0]].fn(this.bot, msg, suffix, this);
        } else return;
      }
    });
